...
def rotacionpivote(lista, pivote, grados):
    for i in range(0, len(lista)):
        ...
        pivx = pivote[0]
        pivy = pivote[1]
        lista[i][0] = round((pivx + ((equis - pivx) * math.cos(math.radians(grados))) - ((ye - pivy) * math.sin(math.radians(grados)))), 2)
        lista[i][1] = round((pivy + ((equis - pivx) * math.sin(math.radians(grados))) + ((ye - pivy) * math.cos(math.radians(grados)))), 2)
...
layout = [[sg.Canvas(key='figCanvas')],
          ...
          [sg.Checkbox('Transformadas Compuestas', key='transcomp'), sg.Button('Aplicar Transformada Compuesta', font=AppFont, disabled=True)],
          ...
          [sg.Input(key='pivoteX', s=15, disabled=True), sg.Input(key='pivoteY', s=15, disabled=True), sg.Input(key='gradosPiv', s=15, disabled=True),
